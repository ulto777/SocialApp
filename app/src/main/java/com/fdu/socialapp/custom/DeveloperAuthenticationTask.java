...
import com.fdu.socialapp.devauth.client.Response;
...
public class DeveloperAuthenticationTask extends
    ...
    protected Void doInBackground(LoginCredentials... params) {
        ...
        Log.d("isSuccessful: ", String.valueOf(isSuccessful));
        userName = params[0].getUsername();

        if (isSuccessful) {
            CognitoSyncClientManager.addLogins(
                    ((DeveloperAuthenticationProvider)
                            CognitoSyncClientManager.credentialsProvider
                                    .getIdentityProvider()).getProviderName(), userName);
